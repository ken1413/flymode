...
| **TCP 配對** | 裝置 A 發送配對請求（TCP port 4827）→ 裝置 B 接受 → 雙方互相加入裝置列表，並交換裝置資訊。 |
...
| **不對外連線** | FlyMode 不會回傳任何資料。唯一的監聽 port 是 TCP 4827（本機配對用）。 |
...
| **4827** | TCP | FlyMode 配對協議 — 裝置發現和配對請求 |
